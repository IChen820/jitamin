...
class ProjectUserOverviewController extends Controller
{
    ...
    public function users()
    {
        ...
        $this->response->html($this->template->render('manage/project_user_overview/tooltip_users', [
    ...
    protected function role($role, $action, $title, $title_user)
    {
        ...
        $paginator = $this->paginator
            ->setUrl('Manage/ProjectUserOverviewController', $action, ['user_id' => $user_id])
            ->setMax(30)
            ->setOrder('projects.name')
            ->setQuery($query)
            ->calculate();

        $this->response->html($this->helper->layout->projectUser('manage/project_user_overview/roles', [
            'paginator' => $paginator,
            'title'     => $title,
            'user_id'   => $user_id,
            'users'     => $users,
    ...
    protected function tasks($is_active, $action, $title, $title_user)
    {
        ...
        $this->response->html($this->helper->layout->projectUser('manage/project_user_overview/tasks', [
            'paginator' => $paginator,
            'title'     => $title,
            'user_id'   => $user_id,
            'users'     => $users,
