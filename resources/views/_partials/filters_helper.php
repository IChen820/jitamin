<?= $this->hook->render('template:app:filters-helper:before', isset($project) ? ['project' => $project] : []) ?>
<div class="dropdown">
    <a href="#" class="dropdown-menu dropdown-menu-link-icon" title="<?= t('Default filters') ?>"><i class="fa fa-filter"></i> <i class="fa fa-caret-down"></i></a>
    <ul>
        ...
        <li><a href="#" class="filter-helper" data-filter="status:open updated:today"><?= t('Tasks updated today') ?></a></li>
        ...
        <li><a href="#" class="filter-helper" data-filter="status:closed due:>=<?= date('Y-m-d', strtotime('Monday this week'))?>"><?= t('Closed tasks due this week') ?></a></li>
...
<?= $this->hook->render('template:app:filters-helper:after', isset($project) ? ['project' => $project] : []) ?>