...
<?php else: ?>
    <table class="table-fixed table-scrolling">
        <tr>
            ...
            <th class="column-10"><?= $subtask_paginator->order(t('Time spent'), \Jitamin\Model\SubtaskTimeTrackingModel::TABLE.'.time_spent') ?></th>
        </tr>
        <?php foreach ($subtask_paginator->getCollection() as $record): ?>
        <tr>
            <td><?= $this->url->link($this->text->e($record['user_fullname'] ?: $record['username']), 'Profile/ProfileController', 'show', ['user_id' => $record['user_id']]) ?></td>
