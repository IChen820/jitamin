...
<?php else: ?>
    ...
    <form class="popover-form" method="post" action="<?= $this->url->href('Task/TaskDuplicationController', 'copy', ['task_id' => $task['id'], 'project_id' => $task['project_id']]) ?>" autocomplete="off">
        ...
        <?= $this->form->select(
            ...
            $values,
            [],
            ['data-redirect="'.$this->url->href('Task/TaskDuplicationController', 'copy', ['task_id' => $task['id'], 'project_id' => $task['project_id'], 'dst_project_id' => 'PROJECT_ID']).'"'],
